...
from opax.models.environment_models.bicyclecar_model import BicycleCarReward
from opax.envs.bicycle_car import BicycleEnv
...
def experiment(
    ...
    reward_model_park = BicycleCarReward(action_cost=action_cost)

    env_kwargs_park = {
        'reward_model': reward_model_park,
    }

    reward_model_forward = BicycleCarReward(action_cost=action_cost, goal=jnp.asarray([2.0, 0.0, 0.0]))
    ...
    env = make_vec_env(env_id=BicycleEnv, wrapper_class=wrapper_cls, n_envs=n_envs, seed=seed,
                       env_kwargs=env_kwargs_forward)

    test_env_forward = make_vec_env(env_id=BicycleEnv, wrapper_class=wrapper_cls_test, n_envs=1,
                                    seed=seed, env_kwargs=env_kwargs_forward)

    test_env_park = make_vec_env(env_id=BicycleEnv, wrapper_class=wrapper_cls_test, n_envs=1,
                                 seed=seed, env_kwargs=env_kwargs_park)
    ...
    if exploration_strategy == 'Mean':
        ...
        dynamics_model_forward = ActiveLearningHUCRLModel(
            ...
            use_al_uncertainties=True,
            deterministic=deterministic,
            lr=lr,
            action_cost=action_cost,
        )

        dynamics_model_park = ActiveLearningHUCRLModel(
            action_space=env.action_space,
            observation_space=env.observation_space,
            num_ensemble=num_ensembles,
            reward_model=reward_model_park,
        ...
        dynamics_model_park = ActiveLearningPETSModel(
            action_space=env.action_space,
            observation_space=env.observation_space,
            num_ensemble=num_ensembles,
            reward_model=reward_model_park,
        ...
        video_prefix += 'PETS'
    elif exploration_strategy == 'OPAX':
        dynamics_model_forward = ActiveLearningHUCRLModel(
            action_space=env.action_space,
            observation_space=env.observation_space,
            num_ensemble=num_ensembles,
        ...
        dynamics_model_park = HUCRLModel(
            action_space=env.action_space,
            observation_space=env.observation_space,
            num_ensemble=num_ensembles,
            reward_model=reward_model_park,
    ...
    dynamics_model = [dynamics_model_forward, dynamics_model_park]
    test_env = [test_env_forward, test_env_park]
