...
# class GPModelParameters(NamedTuple):
#     gp_parameter_state: gpjax.parameters.ParameterState
#     dataset: Dataset
...
#         active_dims = [i for i in range(self.input_dim)]
#         self.kernel = jk.RBF(active_dims=active_dims)
#         prior = gpx.Prior(kernel=self.kernel)
#         self.rng = jax.random.PRNGKey(seed)
#         key, self.rng = jax.random.split(self.rng, 2)
#         self.output_dim = output_dim
#         key = jax.random.split(key, output_dim)
#         likelihood = gpx.Gaussian(num_datapoints=0)
#         posterior = prior * likelihood
#         parameter_state = jax.vmap(gpx.initialise, (None, 0))(posterior, key)
#         dataset = jax.vmap(lambda out: Dataset(X=jnp.empty((1,) + (input_dim,)), y=out.reshape(-1, 1)), in_axes=-1,
#                            out_axes=0)(jnp.empty((1,) + (output_dim,)))
#         self.parameter_state = GPModelParameters(
#             gp_parameter_state=parameter_state,
#             dataset=dataset,
#         )
...
#         self.calculate_calibration_alpha = jax.jit(calculate_calibration_alpha)
#
#     def _predict(self, x, parameter_state: GPModelParameters):
#         out_mean, out_std, y_mean, y_std = jax.vmap(self._predict_single_output, (None, 0, 0),
#                                                     out_axes=(-1, -1, -1,
#                                                               -1))(x,
#                                                                    parameter_state.gp_parameter_state.params,
#                                                                    parameter_state.dataset)
#         return out_mean, out_std, y_mean, y_std
#
#     @functools.partial(jax.jit, static_argnums=(0,))
#     def _predict_single_output(self, x, params, dataset):
#         prior = gpx.Prior(kernel=self.kernel)
#         likelihood = gpx.Gaussian(num_datapoints=dataset.n)
#         posterior = prior * likelihood
#         latent_dist = posterior(params, dataset)(jnp.atleast_2d(x))
#         predictive_dist = likelihood(params, latent_dist)
#         predictive_mean = latent_dist.mean()
#         predictive_std = latent_dist.stddev()
#         return predictive_mean, predictive_std, predictive_dist.mean(), predictive_dist.stddev()
#
#     @functools.partial(jax.jit, static_argnums=(0, 3))
#     def _train_single_fn(self, key, data, num_steps: int = 1500):
#         prior = gpx.Prior(kernel=self.kernel)
...
#         optimiser = optax.adamw(learning_rate=self.lr, weight_decay=self.weight_decay)
#         tx = optax.adamw(learning_rate=optax.piecewise_constant_schedule(0.01, {4000: 1e-1}),
...
#         key = jax.random.split(key, self.output_dim)
#         dataset = jax.vmap(lambda out: Dataset(X=x, y=out.reshape(-1, 1)), in_axes=-1,
#                            out_axes=0)(y)
#
#         def train_fn(rng, data):
#             return self._train_single_fn(
#                 key=rng,
#                 data=data,
#                 num_steps=num_steps,
#             )
#         learned_params = jax.vmap(train_fn)(key, dataset)
#         old_params, trainables, bijectors = self.init_parameter_state.gp_parameter_state.unpack()
#         trained_state = ParameterState(
...
class GPModel(object):
    ...
    def _train(self, x, y, key, num_steps: int = 1000):
        ...
        def train_step(carry, ins):
            ...
            carry = [
                new_opt_state, new_params, key
            ]
            return carry, outs

        carry = [opt_state, params, subkey]
