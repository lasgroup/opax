...
from mbse.utils.vec_env.base_vec_env import (
    CloudpickleWrapper,
    VecEnv,
    VecEnvIndices,
    VecEnvObs,
...
def _worker(
    remote: Connection, parent_remote: mp.connection.Connection, env_fn_wrapper: CloudpickleWrapper
) -> None:
    # Import here to avoid a circular import
    from mbse.utils.vec_env.env_util import is_wrapped
...
class SubprocVecEnv(VecEnv):
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        self.waiting = False
        obs, rews, terminal, truncate, infos = zip(*results)
        return _flatten_obs(obs, self.observation_space), np.stack(rews), np.stack(terminal), np.stack(truncate), infos
